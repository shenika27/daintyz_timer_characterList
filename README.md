...
**신규 테마 출시(권장) = 스킨빌더로 번들 zip 만들어 `_inbox/`에 업로드만** 하면 됩니다 — `character_zip/`·`preview/` 배치와 `catalog.json` 갱신은 GitHub Action이 자동 처리(아래 **PART 3** 참고). 위 폴더들을 손으로 채우는 수동 방식도 그대로 가능합니다.
