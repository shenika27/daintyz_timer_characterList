...
**캐릭터 제작·관리**와 **주문·코드** 두 화면으로 구분합니다.

- 캐릭터 제작·관리는 전체 폭 카드 목록과 전체 폭 편집 화면이 서로 전환되는 구조입니다.
- 편집 화면은 기본정보, 상황별 이미지 11종, 상황별 음성 10종, 미리보기 단계로 구성합니다.
- PC 앱 `VERSION`의 현재 패치 버전과 다음 예정 버전을 읽기 전 UI로 현재 `0.6.7`, 예정 `0.6.8`을 표시합니다.
- 게시 확인 창에서 업데이트 로그를 입력하며 예정 버전은 직접 편집할 수 없습니다.

현재 4단계 1차 UI에서는 캐릭터 ID·이름·운영 상태만 D1에 저장합니다. 이미지·음원 파일 전송과
`character_todo` 저장소 반영, 버전·변경 로그 갱신, GitHub Release 실행은 게시 자동화 단계에서 연결합니다.
그 전까지 게시 확인 버튼은 비활성화됩니다.

### CharacterTodo 판매 코드 운영

`todo-builder`에서 **주문·코드**를 선택하면 판매 관리 화면으로 이동합니다.

- 판매 캐릭터를 `준비 중(DRAFT)`·`판매 중(PUBLISHED)`·`판매 종료(ARCHIVED)` 상태로 등록·수정합니다.
- 네이버 주문번호, 구매자 이메일, 캐릭터, 수량을 입력하면 수량별 최초 코드가 발급됩니다.
- **주문·코드 목록**에서 주문번호·마스킹 이메일·캐릭터로 검색하고 발급·소진·환불 수량을 확인합니다.
- **발급 횟수**를 누르면 수량 단위의 전체 발급 이력과 현재 활성 여부가 팝업으로 표시됩니다.
- 현재 코드는 횟수 제한 없이 재발급할 수 있습니다. 새 코드가 발급되면 기존 코드는 즉시 비활성화됩니다.
- 미사용 활성 코드 폐기와 수량 단위 환불을 지원하며, 환불 시 연결된 코드와 이용권도 폐기됩니다.
