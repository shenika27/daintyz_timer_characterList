...
```
index.html                      ← 🎨 스킨빌더 (브라우저로 열어 번들 zip 생성)
...
1. **스킨빌더**(이 레포 최상위 `index.html`)를 브라우저로 열기
...
> 번들을 풀어 `character/zip/{id}.zip`·`character/preview/{id}/` 배치 + `catalog.json`에 항목 upsert(기존이면 version +1) + 올린 inbox zip 삭제 후 커밋.
...
```json
{
  ...
  "skins": [
    { "skinId": "newchar", "name": "캐릭터 이름", "price": 0, "prestige": false,
      "description": "한 줄 부제", "createdAt": "2026-06-26", "version": 1 }
  ]
}
```
