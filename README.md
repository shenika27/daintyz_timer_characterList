...
- 참고: 기본 감자 스킨은 96×130px(저해상)입니다. 또렷하게 하려면 2~3배(약 200~400px)까지 OK.
...
|---|---|---|---|---|
| 박스 배경 | `background` | `fitXY` (가로·세로 늘어남) | **~600×200px** (가로 약 3:1) | 단순·대칭, 상단(숫자)·하단(버튼) 명암 구분 |
| 가로 구분선 | `dividerHImage` | `fitXY` | **~600×12px** (가로로 긴 띠) | `dividerHHeightDp`를 4~8로 키워야 보임 |
| 세로 구분선 | `dividersImage` | `fitXY` | **~12×96px** (세로로 긴 띠) | `dividersWidthDp`를 4~8로 키워야 보임 |
| 버튼 아이콘 ×5 | `buttons.*` | `fitCenter` (32dp 칸) | **96×96px 정사각** | 투명, 그림은 캔버스 안쪽 ~70% |
| 숫자 | `font` | 그림 아님 | — | 색/크기/패밀리만 지정 |

> 박스·구분선은 `fitXY`로 **늘어나므로 정확한 px보다 비율과 단순함**이 중요합니다(위 값은 또렷함을 위한 기준치). 더 선명하게 하려면 각 1.5~2배까지 키워도 됩니다. 버튼만 `fitCenter`라 비율 유지됩니다.
...
- 버튼 한 칸은 대략 **정사각형**으로 들어갑니다(표시 크기 높이 32dp, 안쪽 5dp 여백 후 `fitCenter`).
- **권장 PNG 크기: 96×96px 정사각**(투명). 더 또렷하게 하려면 128×128px(약 4배)까지 OK.
  - 아이콘 그림은 캔버스 **안쪽 ~70%**에 두고 가장자리에 여백을 주세요(여백 없이 꽉 채우면 옆 버튼과 붙어 보임).
  - 5개 모두 **같은 캔버스 크기**로 맞추면 버튼들이 균일하게 보입니다.
...
- [ ] 버튼 아이콘 투명 배경(RGBA), **96×96px 정사각** 권장 (5개 동일 크기)
