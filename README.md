...
핵심: **박스 + 구분선을 모두 배경 PNG 한 장(`timer_theme`)에 그려 넣습니다.**
버튼이 항상 2개라 어느 상태에서도 선 위치가 같기 때문에, 예전처럼 선을 따로 그릴 필요가 없어졌습니다.

### 영역 구조 (가로)

```
┌───────────────────────────┬─────┐  ← ① 타이머 배경(timer_theme): 박스 + 선까지 한 장에 그림
│                           │  +  │  ← ② 버튼(위)
...
- 타이머 테마 파일은 역할이 드러나게: `timer_theme.png`(배경), `btn_play.png` 등.
...
**타이머 테마**
- [ ] 타이머 배경(`timer_theme`): 박스 + 시간↔버튼 세로선(~80% 지점) + 버튼 사이 가로선을 **한 장에** 포함
...
> `timer` 도 생략 가능하지만(→ 내장 박스 + 기호 버튼), **출시 테마는 `background`(timer_theme)를 무조건 포함**합니다.
>
...
**출시 테마는 `background`(timer_theme)를 무조건 포함**합니다 — 박스·구분선이 이 배경 한 장에 들어갑니다.
...
```
┌───────────────────────────┬─────┐  ← background(timer_theme): 박스 + 선까지 한 장
│                           │  +  │  ← 위 버튼
│         10:00             ├─────┤  ← 버튼 사이 가로선(배경에 그림)
│      (숫자 = font)         │  −  │  ← 아래 버튼
└───────────────────────────┴─────┘
...
1. **스킨빌더**(앱 레포 `tools/skin-builder/index.html`)를 브라우저로 열기
...
> 번들을 풀어 `character_zip/{id}.zip`·`preview/{id}/` 배치 + `catalog.json`에 항목 upsert(기존이면 version +1) + 올린 inbox zip 삭제 후 커밋.
...
├── timer_theme.png                 (타이머 배경 — 박스+선 포함)
├── btn_minus/plus/play/pause/stop.png  (버튼 5종, 투명)
└── timer_font.ttf                  (테마 숫자 폰트, 선택 — font.file로 연결)
```
