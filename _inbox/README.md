...
나머지(압축 풀기 · `character_zip/`·`preview/` 배치 · `catalog.json` 갱신)는 GitHub Action이 자동으로 처리하고, 올린 zip은 처리 후 자동으로 사라집니다.
...
> ⚠ zip은 반드시 **스킨빌더가 만든 번들**이어야 합니다(안에 `character_zip/`·`preview/`·`catalog_entry.json` 포함).
